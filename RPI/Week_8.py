        ...
        while True:
            ...
            if message.startswith("ACK"):
                if self.rs_flag == False:
                    ...
                    self.logger.debug(f"ACK from STM32 received: {message}")
                # ACK only means command received, not completed — do not release lock
                continue

            elif message.startswith("DONE"):
                self.logger.debug(f"DONE from STM32 received: {message}")

            else:
                self.logger.warning(f"Ignored unknown message from STM: {message}")
                continue

            # Release movement lock only on DONE (command completed)
            try:
    ...
    def command_follower(self) -> None:
        ...
        while True:
            ...
            # Snap command
                ...
                obstacle_id_with_signal = command.replace("SNAP", "").strip()
    ...
    def snap_and_rec(self, obstacle_id_with_signal: str) -> None:
        ...
        for retry in range(1, snap_max_retries + 1):
            ...
            if retry <= 2:
                ...
                self.logger.info(
                    f"Attempt {retry}: no match, retrying same settings..."
                )
            elif retry <= 4:
                self.logger.info(f"Image recognition results: {results}")
                self.logger.info(
                    f"Attempt {retry}: no match, lowering shutter speed..."
                )
                speed -= 1
            elif retry == 5:
                self.logger.info(f"Image recognition results: {results}")
                self.logger.info(f"Attempt {retry}: no match, raising shutter speed...")
                speed += 3
        ...
        result_image_id = results["image_id"]
        if isinstance(result_image_id, list):
            result_image_id = result_image_id[0] if result_image_id else "NA"
        self.logger.info(
            f"Image recognition results: {results} ({SYMBOL_MAP.get(str(result_image_id))})"
        )

        if result_image_id == "NA":
        ...
        self.android_queue.put(
            AndroidMessage(
                "image-rec",
                {
                    "image_id": SYMBOL_MAP.get(
                        str(result_image_id), str(result_image_id)
                    ),
                    "obstacle_id": results.get("obstacle_id", "NA"),
                },
            )
        )

    def request_algo(self, data, robot_x=0, robot_y=0, robot_dir=0, retrying=False):
        """
